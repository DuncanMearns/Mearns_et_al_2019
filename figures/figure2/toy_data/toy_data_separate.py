...
from datasets.svd_toy_data.separated_states import *
...
if __name__ == "__main__":
    ...
    for c, state in zip(('c', 'm', 'y'), states):
        kde = KernelDensity(bandwidth=0.05).fit(points[np.isin(cluster_labels, state)])
    ...
    ax.set_yticks([])
    open_ax(ax)
    save_fig(fig1, 'figureS2', 'separated_ground_truth')

    for i, dataset in enumerate([transition_structure_1, transition_structure_2]):

        # Generate transition matrix
        data = ToyData(**dataset)
        T = data.markov_process(n_transitions=2000)
        # Add noise
        ...
        analysis.dendrogram()
        predicted_labels = sch.fcluster(analysis.Y, 3, criterion='maxclust') - 1
        ...
        save_fig(fig2, 'figureS2', 'separated_singular_values_{}'.format(i))

        # Plot transition space
        fig3, ax = plt.subplots(figsize=(1, 1))
        ax.scatter(*analysis.q[:, :2].T, c=predicted_labels, cmap='brg', s=3, lw=0)
        open_ax(ax)
        ax.set_xticks([])
        ax.set_yticks([])
        save_fig(fig3, 'figureS2', 'separated_transition_space_{}'.format(i))

        # Plot transition matrix
        fig4, ax = plt.subplots(figsize=(1, 1))
        ax.imshow(M, cmap='plasma', vmin=0, vmax=0.05)
        ax.set_xticks([])
        ax.set_yticks([])
        save_fig(fig4, 'figureS2', 'separated_transition_matrix_{}'.format(i), ext='png')

        # Plot identified clusters
        fig5, ax = plt.subplots(figsize=(1, 1))
        ax.scatter(*points.T, c=predicted_labels, s=3, lw=0, cmap='brg')
        ax.set_xticks([])
        ax.set_yticks([])
        open_ax(ax)
        save_fig(fig5, 'figureS2', 'separated_clustered_{}'.format(i))
        ...
        ax.set_xticks([])
        ax.set_yticks([])
        save_fig(fig6, 'figureS2', 'separated_confusion_matrix_{}'.format(i))
