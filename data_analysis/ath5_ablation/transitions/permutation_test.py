...
if __name__ == "__main__":
    ...
    control_matrices = np.load(os.path.join(transition_directory, 'control', 'transition_matrices.npy'))
    ablated_matrices = np.load(os.path.join(transition_directory, 'ablated', 'transition_matrices.npy'))

    timer = Timer()
    timer.start()
    dot_products = compare_transition_modes(ablated_matrices, control_matrices, exact=False, n_permutations=100)
    np.save(os.path.join(transition_directory, 'compare_control_ablated.npy'), dot_products)
