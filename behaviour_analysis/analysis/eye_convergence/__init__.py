from .eye_convergence_analyser import EyeConvergenceAnalyser
from ...experiments import TrackingExperiment2D
from ...manage_files import get_files
from ...miscellaneous import print_heading, print_subheading
import pandas as pd
import numpy as np
import os
...
def import_eye_angles(*paths):
    ...
    for i, path in enumerate(paths):
        ...
        eye_angles = tracked[['right', 'left']].applymap(np.degrees)
        fish_angles.append(eye_angles)
    fish_angles = pd.concat(fish_angles, ignore_index=True, axis=0)
    fish_angles['convergence'] = (fish_angles['right'] - fish_angles['left'])
    print '\ndone!\n'
    return fish_angles
...
class EyeTrackingData(object):
    ...
    def from_experiment(cls, path):
        ...
        if isinstance(path, str):
            experiment = TrackingExperiment2D(path)
            experiment.open()
        else:
            experiment = path
        eye_data = {}
        for idx, fish_info in experiment.data.iterrows():
            ...
            kinematics_directory = os.path.join(experiment.directory, fish_info.kinematics_directory)
            kinematics_files, kinematics_paths = get_files(kinematics_directory)
            fish_angles = import_eye_angles(*kinematics_paths)
